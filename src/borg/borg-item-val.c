...
static int borg_lookup_sval_fail(int tval, const char *name)
{
    ...
    /* Try the English name on the current (language-specific) data */
    sval = lookup_sval(tval, name);
    if (sval >= 0) return sval;

    /* Try the translated name (for Spanish via es.po) */
    sval = lookup_sval(tval, _(name));
    if (sval >= 0) return sval;

    /* Fallback: open the English object.txt directly and find the sval */
    {
        char objpath[1024];
        char base_dir[1024];
        ang_file *fh;
        ...
        /* Build path to English gamedata: go up one level from
         * ANGBAND_DIR_GAMEDATA (e.g. lib/gamedata/es/ -> lib/gamedata/)
         * then append "en/object.txt" */
        path_build(base_dir, sizeof(base_dir), ANGBAND_DIR_GAMEDATA, "..");
        path_build(base_dir, sizeof(base_dir), base_dir, "en");
        path_build(objpath, sizeof(objpath), base_dir, "object.txt");

        fh = file_open(objpath, MODE_READ, FTYPE_TEXT);
        if (fh) {
            while (file_getl(fh, line, sizeof(line))) {
                ...
                for (i = strlen(line); i > 0 && (line[i-1] == '\n' || line[i-1] == '\r' || line[i-1] == ' '); i--)
                    line[i-1] = '\0';

                /* type: <tval name> - sets the current tval */
                if (strncmp(line, "type:", 5) == 0) {
                    const char *tval_name = line + 5;
                    while (*tval_name == ' ') tval_name++;
                    cur_tval = tval_find_idx(tval_name);
                    sval_count = 0;
                    continue;
                }

                /* name: <object name> - counts as an object */
                if (strncmp(line, "name:", 5) == 0 && cur_tval >= 0) {
                    const char *obj_name = line + 5;
                    char short_name[1024];

                    while (*obj_name == ' ') obj_name++;
                    object_short_name(short_name, sizeof(short_name), obj_name);

                    if (cur_tval == want_tval) {
    ...
    borg_note(
        format("** Borg: sval lookup failure - %s ", name));
    return -1;
}
...
void borg_init_item_val(void)
{
    ...
    sv_wand_confuse_monster = borg_lookup_sval_fail(tval, "Confuse Monster");
    ...
    sv_wand_elec_ball       = borg_lookup_sval_fail(tval, "Lightning Balls");
