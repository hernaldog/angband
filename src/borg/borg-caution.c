...
static bool borg_heal(int danger)
{
    ...
    if (borg.trait[BI_ISCONFUSED]) {
        ...
        if (danger < borg.trait[BI_CURHP] + csw_heal
            ...
            borg_note(_("# Fixing Confusion. Level 2"));
            return true;
        }
        ...
        if (borg_equips_staff_fail(sv_staff_teleportation)) {
            ...
            if ((borg_activate_failure(TV_STAFF, sv_staff_teleportation) > 650)
                ...
                && (borg_quaff_crit(true)
                    || borg_quaff_potion(sv_potion_healing))) {
                borg_note(_("# Fixing Confusion. Level 3"));
                return true;
            }
             ...
             */
                ...
                borg_note(_("# Too dangerous to fix Confusion. Level 4"));
                return false;
            }
            ...
            if ((borg_quaff_crit(true)
                    || borg_quaff_potion(sv_potion_cure_serious)
                    || borg_quaff_potion(sv_potion_healing))) {
                borg_note(_("# Fixing Confusion. Level 5"));
                return true;
            }
        }
    }
    ...
    if ((borg.trait[BI_ISBLIND] || borg.trait[BI_ISCONFUSED])
        ...
        && borg_quaff_potion(sv_potion_star_healing)) {
        borg_note(_("# Fixing Confusion/Blindness."));
        return true;
    }
    ...
    if (borg_fighting_unique >= 10) {
        if (borg.trait[BI_CURHP] <= 700
            ...
            borg_note(_("# Heal in Mission Combat."));
            return true;
        }
    }
    ...
    if (borg.trait[BI_CURSP] < (borg.trait[BI_MAXSP] / 5)
        ...
        if (borg_use_staff_fail(sv_staff_the_magi)
            ...
            borg_note(_("# Use Staff of the Magi"));
            return true;
        }
    }
    ...
    if (stats_needing_fix >= 5 && borg_fighting_unique >= 10
        ...
        borg_note(_("# Trying to fix stats in combat."));
        return true;
    }
    ...
    if (borg.trait[BI_ISPOISONED]
        ...
        if ((borg_spell_legal(CURE_POISON) || borg_spell_legal(HERBAL_CURING))
            && (borg_quaff_potion(sv_potion_restore_mana)
                || borg_activate_item(act_restore_mana))) {
            borg_note(_("# Healing next round."));
            return true;
        }
    }
    ...
    if (borg.trait[BI_ISPOISONED]
        ...
        if (borg_spell(CURE_POISON) || borg_spell(HERBAL_CURING)
            ...
            /* Flee! */
            borg_note(_("# Emergency Poison Cure! Gasp!...."));
        ...
        if (borg.trait[BI_CURHP] < borg.trait[BI_MAXHP] / 20
            && (borg_quaff_potion(sv_potion_star_healing)
                || borg_quaff_potion(sv_potion_life)
                || borg_quaff_potion(sv_potion_healing))) {
            borg_note(_("# Healing. Heal section."));
            return true;
        }
    ...
    if (borg.trait[BI_ISCUT]
        ...
        if (borg_spell(MINOR_HEALING)) {
            ...
            /* Flee! */
            borg_note(_("# Emergency Wound Patch! Gasp!...."));
...
bool borg_caution(void)
{
    ...
    if (borg.trait[BI_CDEPTH] == 100 && borg_t - borg_began < 10
        ...
        if (borg.ready_morgoth <= 0 && !borg.trait[BI_KING]) {
            ...
            if (borg_spell(TELEPORT_LEVEL)
                ...
                borg_note(_("# Going up a dungeon level (Not ready for Morgoth)"));
                return true;
            }
            ...
            if (!borg.goal.leaving) {
                /* Note */
                borg_note(_("# Abandoning (Not ready for Morgoth now)"));
    ...
    if ((borg.escapes > 3 && !unique_on_level && !borg.ready_morgoth)
        ...
        if (borg.trait[BI_CDEPTH] <= 98) {
            ...
            if (!borg.goal.leaving) {
                /* Note */
                borg_note(_("# Abandoning (Too many escapes)"));
            ...
            if (!borg.goal.fleeing && borg.escapes > 3) {
                /* Note */
                borg_note(_("# Fleeing (Too many escapes)"));
    ...
    if (scaryguy_on_level) {
        /* Note */
        borg_note(_("# Terrifying type on level."));

        /* Start leaving */
        if (!borg.goal.leaving) {
            /* Note */
            borg_note(_("# Abandoning (Terrifying type on level)"));
        ...
        if (!borg.goal.fleeing) {
            /* Note */
            borg_note(_("# Fleeing (Terrifying type on level)"));
    ...
    if (borg.goal.ignoring) {
        /* Note */
        borg_note(_("# Ignoring combat with monsters."));
    }

    /* Note if ignorig messages */
    if (borg_dont_react) {
        borg_note(_("# Borg is ignoring messages."));
    }
    ...
    if ((((pos_danger > avoidance / 10)
        ...
        if (borg.temp.shield) {
            borg_note(_("# Protected by Mystic Shield"));
        }
        if (borg.temp.prot_from_evil) {
            borg_note(_("# Protected by Protection from Evil"));
        }
        if (borg_morgoth_position) {
            borg_note(_("# Protected by Sea of Runes."));
        }
        if (borg_fighting_unique >= 10) {
            borg_note(_("# Mission Combat."));
        }
        if (borg_as_position) {
            borg_note(_("# Protected by anti-summon corridor."));
        }
    }
    /* Comment on glyph */
    if (track_glyph.num) {
        ...
        for (i = 0; i < track_glyph.num; i++) {
            ...
            if ((track_glyph.y[i] == borg.c.y)
                ...
                /* if standing on one */
                borg_note(format(_("# Standing on a Glyph")));
            }
        }
    }
    /* Comment on stair */
    if (track_less.num) {
        ...
        for (i = 0; i < track_less.num; i++) {
            ...
            if ((track_less.y[i] == borg.c.y)
                ...
                /* if standing on one */
                borg_note(format(_("# Standing on upward stairs")));
                on_upstair = false;
            }
        }
    }
    ...
    if (track_more.num) {
        ...
        for (i = 0; i < track_more.num; i++) {
            ...
            if ((track_more.y[i] == borg.c.y)
                ...
                /* if standing on one */
                borg_note(format(_("# Standing on downward stairs")));
                on_dnstair = false;
            }
        }
    }
    ...
    if (borg.goal.recalling && borg_check_rest(borg.c.y, borg.c.x)
        ...
        borg_note(_("# Resting for Return."));
        borg_keypress('R');
        borg_keypress('5');
        borg_keypress('0');
        borg_keypress('0');
    ...
    /* Excessive danger */
        ...
        if (!borg.goal.fleeing && !borg_fighting_unique
            ...
            /* Note */
            borg_note(_("# Fleeing (excessive danger)"));
    ...
    /* Potential danger (near death) in town */
        ...
        if (!borg.goal.leaving) {
            /* Flee! */
            borg_note(_("# Abandoning (potential danger)"));
            ...
            && on_upstair)) /* danger and standing on stair */
    {
        if (borg.ready_morgoth == 0 && !borg.trait[BI_KING]
            ...
            if (scaryguy_on_level)
                borg_note(_("# Fleeing and abandoning level. (terrifying type)"));
            if (borg.goal.fleeing_lunal)
                borg_note(_("# Fleeing and abandoning level. (moon_flee)"));
            ...
            if (pos_danger > avoidance && borg.trait[BI_CLEVEL] <= 49
                ...
                borg_note(_("# Abandoning level,  Some danger but I'm on stairs."));
        }
    ...
    if (borg.stair_less && !OPT(player, birth_force_descend)) {
        ...
        if ((ag->feat == FEAT_LESS) || on_upstair) {
            /* Log it */
            borg_note(format(_("# Abandoning via upward stairs.")));
    ...
    if (!borg.trait[BI_LIGHT]) /* No Lite, AND Not Glowing */
    {
        ...
        if (need == BORG_MET_NEED)
            ...
            if (!borg.goal.leaving) {
                /* Flee */
                borg_note(_("# Abandoning (need fuel)"));
    ...
    if (borg.trait[BI_ISWEAK]) {
        ...
        if (borg.trait[BI_CDEPTH]) {
            ...
            if (!borg.goal.leaving) {
                /* Flee */
                borg_note(_("# Abandoning (need food)"));
    ...
    if (breeder_level && borg.trait[BI_CLEVEL] < 15) {
        ...
        if (!borg.goal.fleeing) {
            /* Flee */
            borg_note(_("# Fleeing (level with breeders)"));
    ...
    if ((track_less.num || track_more.num)
        ...
        if (b_j < 3 && b_j != -1 && pos_danger < borg.trait[BI_CURHP]) {
            ...
            if (borg_flow_stair_less(GOAL_FLEE, false)) {
                /* Note */
                borg_note(_("# Desperate for Stairs (one)"));
        ...
        if (b_j <= 2 && b_j != -1) {
            ...
            if (borg_flow_stair_less(GOAL_FLEE, false)) {
                /* Note */
                borg_note(_("# Desperate for Stairs (two)"));
        ...
        if (b_j < 20 && b_j != -1 && scaryguy_on_level
            ...
            if ((borg.trait[BI_CDEPTH] <= 5 && borg.trait[BI_CDEPTH] != 0
                ...
                if (borg_flow_stair_less(GOAL_FLEE, false)) {
                    /* Note */
                    borg_note(_("# Desperate for Stairs (three)"));
        ...
        if (b_m <= 2 && b_m != -1) {
            ...
            if (borg_flow_stair_more(GOAL_FLEE, false, false)) {
                /* Note */
                borg_note(_("# Desperate for Stairs (four)"));
    ...
    if (((pos_danger > (avoidance * 4 / 10) && !nasty && !borg.no_retreat)
        ...
        if (b_i >= 0) {
            ...
            /* Note */
            borg_note(format(_("# Backing away to %d,%d (%d > %d)"), borg.goal.g.x,
    ...
    if (((borg.trait[BI_CURHP] < borg.trait[BI_MAXHP] / 3)
        ...
        if (borg.trait[BI_CDEPTH] && (randint0(100) < 25)) {
            ...
            if (!borg.goal.leaving) {
                /* Flee */
                borg_note(_("# Abandoning (few hit points)"));
            ...
            if (!borg.goal.fleeing) {
                /* Flee */
                borg_note(_("# Fleeing (few hit points)"));
    ...
    if ((borg.trait[BI_ISCUT] || borg.trait[BI_ISPOISONED])
        ...
        if (borg.trait[BI_CDEPTH] && (randint0(100) < 25)) {
            ...
            if (!borg.goal.leaving) {
                /* Flee */
                borg_note(_("# Abandoning (bleeding/poison)"));
            ...
            if (!borg.goal.fleeing) {
                /* Flee */
                borg_note(_("# Fleeing (bleeding/poison)"));
    ...
    if ((borg.trait[BI_CURHP] < borg.trait[BI_MAXHP] / 10
        ...
        (borg_quaff_potion(sv_potion_star_healing)
            || borg_quaff_potion(sv_potion_healing)
            || borg_quaff_potion(sv_potion_life))) {
        borg_note(_("# Using Easy Heal Reserve."));
        return true;
    }
    ...
    if (borg.goal.fleeing && !borg.goal.fleeing_munchkin
        ...
        if (borg_recall()) {
            /* Note */
            borg_note(_("# Fleeing the level (return)"));
    ...
    if (borg.goal.recalling && (pos_danger > avoidance * 2)) {
        if (!borg.trait[BI_ISCONFUSED] && !borg.trait[BI_ISBLIND]
            ...
            && (borg_quaff_potion(sv_potion_restore_mana)
                || borg_activate_item(act_restore_mana))) {
            borg_note(_("# Gaining time waiting for Return.(1)"));
            return true;
        }
        ...
        if (borg_caution_phase(50, 1)
            ...
            borg_note(_("# Gaining time waiting for Return.(2)"));
            return true;
        }
        ...
        if ((borg.trait[BI_MAXHP] - borg.trait[BI_CURHP] < 100)
            && (borg_quaff_crit(true)
                || borg_quaff_potion(sv_potion_cure_serious)
                || borg_quaff_potion(sv_potion_cure_light))) {
            borg_note(_("# Gaining time waiting for Return.(3)"));
            return true;
        }
        ...
        if ((borg.trait[BI_MAXHP] - borg.trait[BI_CURHP] > 150)
            && (borg_zap_rod(sv_rod_healing)
                ...
                || borg_quaff_potion(sv_potion_cure_serious)
                || borg_quaff_potion(sv_potion_cure_light))) {
            borg_note(_("# Gaining time waiting for Return.(4)"));
            return true;
        }
    }
